...
(!) Флаг httpOnly  не разрешает изменение cookie  через JS. Это компетенция только сервера. Refresh  токен записывается в БД на сервере и формируется **сессия**. PS: также можно записывать IP  адрес устройства или браузер и, если обнаружен другой ip, отправлять уведомление безопасности пользователю.
